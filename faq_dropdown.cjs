...
// 2. Replace the FAQ grid content with mapped array
const faqOriginalStartStr = '<div className="faq-grid" style={{ display: \'flex\', flexDirection: \'column\', gap: \'15px\', marginTop: \'40px\' }}>';
const faqOriginalEndStr = '</div>\n                </div>\n                <script';

const faqStartIndex = content.indexOf(faqOriginalStartStr);
const faqEndIndex = content.indexOf(faqOriginalEndStr, faqStartIndex);

if (faqStartIndex !== -1 && faqEndIndex !== -1) {
    const faqReplacementJSX = `<div className="faq-grid" style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginTop: '40px' }}>
                        {[
                            {
    ...
    content = content.substring(0, faqStartIndex) + faqReplacementJSX + '\n                ' + content.substring(faqEndIndex + 6); // +6 to skip </div>
    
    fs.writeFileSync('app/page.tsx', content);
    console.log("Replaced FAQ with Dropdown version");
} else {
    console.log("Could not find FAQ boundaries");
}
