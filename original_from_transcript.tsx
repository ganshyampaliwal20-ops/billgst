...
export default function KharchaTrackerAdvanced({ initialData = {} as any, onChange = (d: any) => {}, whatsappNumber = "" }) {
  ...
  const categories = useMemo(() => {
    const set = new Set([...Object.keys(budgets), ...expenses.map((e) => e.category)]);
  ...
  useEffect(() => {
    onChange({ income, expenses, fixedExpenses, budgets, savingGoal, monthlyHistory, lastMonthExpense });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [income, expenses, fixedExpenses, budgets, savingGoal, monthlyHistory, lastMonthExpense]);
  ...
  const exportExcel = useCallback(() => {
    ...
    expenses.forEach((e) => rows.push([e.category, e.amount, e.date]));
  ...
  const exportPDF = useCallback(() => {
    const rows = expenses.map((e) => `<tr><td>${e.category}</td><td>${formatINR(e.amount)}</td><td>${e.date}</td></tr>`).join("");
    ...
    <div style={S.page}>
      ...
      <Card title="This month's income / salary">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginTop: 20, textAlign: "center" }}>
          ...
          <Stat label="Spent" value={formatINR(totalSpent)} color={T.rose} />
          <Stat label="Saved" value={formatINR(saved)} color={T.emerald} />
        </div>
      ...
      <Card title="Add new expense">
        <div style={S.col}>
          ...
          <button onClick={handleAddExpense} style={S.gradientBtn}>+ Add Expense</button>
        </div>
      </Card>
      ...
      {sortedCategories.length > 0 && (
        <Card title="Where is the money going">
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            <Donut segments={sortedCategories.map(([cat, val]) => ({ value: val as number, color: colorFor(cat, categories) }))} total={totalSpent} centerLabel={formatINR(totalSpent)} centerSub="Total Expense" />
      ...
      <Card title="📊 Kharcha chart" subtitle="Pichle mahino ke mukable is mahine ka kharcha">
        ...
        <div style={{ display: "flex", gap: 16, marginTop: 12, fontSize: 12, color: T.textDim }}>
          <span style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ width: 10, height: 10, borderRadius: 3, background: "#ec4899" }} /> Spent</span>
      ...
      {/* Budget alert */}
      <Card title="🔔 Budget alert" subtitle="Set budget for category, get warned at 80%">
        <div style={S.col}>
      ...
      <Card title={`📅 Din-wise kharcha — ${monthLabel(thisMonth)}`}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 6 }}>
          {dayWise.map(({ day, total }) => {
            ...
            return (
              <div key={day} title={formatINR(total)} style={{ aspectRatio: "1", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, background: bg, color: fg }}>
                {day}
              </div>
            );
          })}
      ...
      <Card title="This month's expenses">
        {expenses.length === 0 ? (
          <p style={{ fontSize: 14, color: T.textDim, margin: 0 }}>Abhi koi expense add nahi hua.</p>
        ) : (
            ...
            {expenses.map((e) => (
              <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: T.fieldBg, borderRadius: 12, padding: "10px 16px", fontSize: 14 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <button onClick={() => handleDeleteExpense(e.id)} style={S.smallDelete}>Delete</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Report download */}
      <Card title="📄 Report download karein" subtitle="Full month's account in a PDF report — for your record or CA">
        <div style={S.row}>
