...
export async function POST(request: Request) {
    ...
    try {
        ...
        const queryParams = [
            ...
            data.bank_name || data.business_bank_name || '',
            data.account_no || data.business_account_no || '',
            data.ifsc_code || data.business_ifsc_code || '',
            data.branch_name || data.business_branch_name || '',
            data.account_holder || data.business_account_holder || '',
            data.show_bank_details ?? true,
            userId,
            data.invoice_template || 'TEMPLATE_1',
            data.invoice_table_format || 'FORMAT_1',
            data.signature || data.business_signature || null,
            data.logo_position || data.business_logo_position || 'RIGHT',
            data.autoRemindersEnabled ?? data.auto_reminders_enabled ?? false,
            data.reminderFrequency ?? data.reminder_frequency ?? 3,
            data.reminderTime || data.reminder_time || '10:00',
            data.whatsappBotEnabled ?? data.whatsapp_bot_enabled ?? false,
            data.whatsappSenderNumber || data.whatsapp_sender_number || '',
            data.whatsappApiKey || data.whatsapp_api_key || '',
            data.whatsappApiUrl || data.whatsapp_api_url || '',
            ...
            data.invoice_pdf_size || 'A4',
            data.expense_delete_pin || null,
            data.language || 'en',
            data.nic_username || data.business_nic_username || null,
            data.nic_password || data.business_nic_password || null
        ];
...
function normalizeProfile(dbRow: any, userId: string) {
    ...
    return {
        ...
        gstin: rawGst,
        gst: rawGst,
        business_gstin: rawGst,
        address: dbRow.business_address || '',
        business_address: dbRow.business_address || '',
        phone: dbRow.business_phone || '',
        business_phone: dbRow.business_phone || '',
        email: dbRow.business_email || '',
        business_email: dbRow.business_email || '',
        logo: dbRow.business_logo || null,
        business_logo: dbRow.business_logo || null,
        upi_id: dbRow.business_upi_id || '',
        business_upi_id: dbRow.business_upi_id || '',
        owner_name: dbRow.business_owner_name || '',
        business_owner_name: dbRow.business_owner_name || '',
        bank_name: dbRow.business_bank_name || '',
        business_bank_name: dbRow.business_bank_name || '',
        account_no: dbRow.business_account_no || '',
        business_account_no: dbRow.business_account_no || '',
        ifsc_code: dbRow.business_ifsc_code || '',
        business_ifsc_code: dbRow.business_ifsc_code || '',
        branch_name: dbRow.business_branch_name || '',
        business_branch_name: dbRow.business_branch_name || '',
        ...
        business_signature: dbRow.business_signature || null,
        ...
        business_terms_and_conditions: dbRow.business_terms_and_conditions || '',
        ...
        nic_username: dbRow.nic_username || '',
        nic_password: dbRow.nic_password || '',
        id: userId
    };
}
...
async function runMigration(client: any) {
    return await client.query(`
        ...
        ADD COLUMN IF NOT EXISTS nic_username VARCHAR(100),
        ADD COLUMN IF NOT EXISTS nic_password VARCHAR(255);
    `);
}
