...
export default function AdminPaymentsPage() {
    ...
    const canAccessAdmin = isOwnerRole(role) || adminEmails.includes(session?.user?.email || '');
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 p-2">
            <div className="max-w-6xl mx-auto space-y-4">
                ...
                {/* Header Section */}
                <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-r from-indigo-900 via-indigo-800 to-violet-800 p-2 sm:p-6 shadow-xl text-center flex flex-col items-center justify-center">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <div className="lg:col-span-4 space-y-4">
                        ...
                        {/* Assign Role Card */}
                        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-[8px] sm:p-6 shadow-xl shadow-slate-200/40 relative overflow-hidden group">
                            ...
                            <div className="relative z-10 p-[8px]">
                                ...
                                <div className="space-y-4 sm:space-y-5">
                                    <div className="space-y-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-xl border border-slate-200 bg-slate-50 p-[8px] sm:px-4 sm:py-3.5 text-sm outline-none transition-all focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10"
                                        />
                                    </div>
                                    <div className="space-y-1.5">
                                        ...
                                        <select
                                            ...
                                            className="w-full rounded-xl border border-slate-200 bg-slate-50 p-[8px] sm:px-4 sm:py-3.5 text-sm font-semibold text-slate-700 outline-none transition-all focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 appearance-none"
                                        >
                                    ...
                                    <button
                                        ...
                                        className="w-full rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 p-[8px] sm:px-5 sm:py-4 text-sm font-bold text-white shadow-lg shadow-indigo-200 transition-all hover:scale-[1.02] hover:shadow-indigo-300 disabled:scale-100 disabled:opacity-70 disabled:cursor-not-allowed"
                                    >
                                        {assignLoading ? 'Assigning...' : 'Assign Role Now'}
                                    </button>

                                    {assignResult && (
                                        <div className={`mt-2 rounded-lg border p-[8px] text-xs font-semibold ${assignResult.toLowerCase().includes('fail') || assignResult.toLowerCase().includes('error') ? 'bg-rose-50 border-rose-100 text-rose-600' : 'bg-emerald-50 border-emerald-100 text-emerald-700'}`}>
                                            {assignResult}
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-[8px] sm:p-6 shadow-xl shadow-slate-200/40">
                            <div className="p-[8px] sm:p-0">
                                <div className="flex items-center justify-between mb-4 sm:mb-6">
                                    ...
                                    <span className="bg-amber-100 text-amber-700 font-black text-[10px] sm:text-xs px-[8px] py-1 rounded-lg">{pending.length} Pending</span>
                                </div>
                                ...
                                {loading ? (
                                    ...
                                    <div className="space-y-2 sm:space-y-3 max-h-[300px] sm:max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                                        {pending.map((user) => (
                                            <div key={user.id} className="bg-slate-50 border border-slate-100 rounded-xl p-[8px] sm:p-4 transition-all hover:border-indigo-200">
                                                ...
                                                <div className="bg-white border border-slate-200 rounded-lg p-[8px] flex justify-between items-center mb-2 sm:mb-3 text-[10px] sm:text-xs">
                                                ...
                                                <div className="flex gap-[8px] sm:gap-2">
                                                    <button onClick={() => handleAction(user.id, 'REJECT')} className="flex-1 p-[8px] bg-white border border-rose-200 text-rose-600 rounded-lg sm:rounded-xl font-bold hover:bg-rose-50 transition-colors text-[10px] sm:text-xs">
                                                        Reject
                                                    </button>
                                                    <button onClick={() => handleAction(user.id, 'APPROVE')} className="flex-1 p-[8px] bg-green-500 text-white rounded-lg sm:rounded-xl font-bold hover:bg-green-600 shadow-sm shadow-green-200 transition-colors text-[10px] sm:text-xs">
                                                        Approve
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    <div className="lg:col-span-8">
                        <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-[8px] sm:p-8 shadow-xl shadow-slate-200/40 h-full">
                            <div className="p-[8px] sm:p-0">
                                ...
                                {roleListLoading ? (
                                    ...
                                    <div className="text-center p-[8px] sm:py-20 bg-slate-50 rounded-2xl sm:rounded-3xl border border-slate-100 border-dashed">
                                        ...
                                        <table className="w-full text-left whitespace-nowrap">
                                            <thead>
                                                <tr className="bg-slate-50/80 border-y border-slate-100 text-[10px] sm:text-xs uppercase tracking-wider text-slate-500 font-bold">
                                                    <th className="p-[8px] sm:px-4 sm:py-4 rounded-l-lg sm:rounded-l-xl">Member</th>
                                                    <th className="p-[8px] sm:px-4 sm:py-4">Role</th>
                                                    <th className="p-[8px] sm:px-4 sm:py-4 text-right rounded-r-lg sm:rounded-r-xl">Actions</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100/50">
                                                {roleList.map((item) => (
                                                    <tr key={`${item.source}-${item.id}`} className="hover:bg-slate-50/50 transition-colors group">
                                                        <td className="p-[8px] sm:px-4 sm:py-4">
                                                            <div className="flex items-center gap-2 sm:gap-3">
                                                                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold uppercase shrink-0 text-xs sm:text-base">
                                                        ...
                                                        <td className="p-[8px] sm:px-4 sm:py-4 text-right">
                                                            <div className="flex items-center justify-end gap-[8px] sm:gap-2">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setAssignEmail(item.email || item.phone || item.name || '')}
                                                                    className="rounded bg-slate-100 p-[8px] sm:px-3 sm:py-2 text-[10px] sm:text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition"
                                                                >
                                                                    Edit
                                                                </button>
                                                                <button
                                                                    ...
                                                                    className="rounded bg-indigo-600 p-[8px] sm:px-3 sm:py-2 text-[10px] sm:text-xs font-bold text-white shadow-sm hover:bg-indigo-700 transition"
                                                                >
                                                                    Activity
                                                                </button>
                                                            </div>
...
export function ActivityModal({ open, onClose, identifier, source }: any) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-[8px] sm:p-6 opacity-100 transition-opacity">
            ...
            <div className="relative w-full max-w-4xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
                ...
                {/* Modal Header */}
                <div className="bg-slate-50 border-b border-slate-100 p-[8px] sm:px-6 sm:py-5 flex items-center justify-between shrink-0">
                    <div>
                ...
                {/* Filters */}
                <div className="p-[8px] sm:px-6 sm:py-4 border-b border-slate-100 bg-white flex flex-wrap items-center justify-between gap-[8px] sm:gap-4 shrink-0">
                    <div className="flex flex-wrap items-center gap-[8px] sm:gap-3 w-full sm:w-auto">
                        <div className="flex items-center gap-[8px] sm:gap-2 bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl p-[8px] sm:px-3 sm:py-2 flex-1 sm:flex-none">
                        ...
                        <button onClick={() => fetchData(startDate, endDate)} className="rounded-lg sm:rounded-xl bg-indigo-600 text-white p-[8px] sm:px-5 sm:py-2 text-[10px] sm:text-sm font-bold shadow-md shadow-indigo-200 hover:bg-indigo-700 transition w-full sm:w-auto">Apply Filter</button>
                    </div>
                    <button onClick={generatePDF} className="rounded-lg sm:rounded-xl bg-slate-800 text-white p-[8px] sm:px-5 sm:py-2 text-[10px] sm:text-sm font-bold shadow-md hover:bg-slate-900 transition flex items-center justify-center gap-1 sm:gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                        <span>📄</span> Export PDF
                    </button>
                </div>

                {/* Content */}
                <div className="p-[8px] sm:p-6 overflow-y-auto bg-slate-50/50 flex-1">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center p-[8px] sm:py-20 space-y-4">
                        ...
                        <div className="bg-rose-50 border border-rose-100 text-rose-600 p-[8px] sm:p-6 rounded-xl sm:rounded-2xl text-center font-bold text-xs sm:text-base">
                            {data.error}
                        </div>
                    ) : (
                        <div className="space-y-[8px] sm:space-y-8">
                            ...
                            {(data?.user || data?.staff) && (
                                <div className="bg-white rounded-xl sm:rounded-2xl p-[8px] sm:p-5 border border-slate-100 flex items-center gap-[8px] sm:gap-4 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-[8px] sm:gap-6">
                                {/* Invoices */}
                                {data?.invoices && data.invoices.length > 0 && (
                                    <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                        <div className="bg-slate-50 p-[8px] sm:px-4 sm:py-3 border-b border-slate-100 font-bold text-slate-700 flex justify-between items-center text-xs sm:text-base">
                                            🧾 Invoices
                                            <span className="bg-slate-200 text-slate-600 text-[10px] sm:text-xs px-[8px] py-0.5 rounded-full">{data.invoices.length}</span>
                                        </div>
                                        <div className="p-[8px] space-y-[8px] max-h-48 sm:max-h-64 overflow-y-auto custom-scrollbar">
                                            {data.invoices.map((inv: any) => (
                                                <div key={inv.id} className="p-[8px] sm:p-3 border border-slate-100 rounded-lg sm:rounded-xl hover:border-indigo-100 bg-white transition-colors">
                                ...
                                {data?.expenses && data.expenses.length > 0 && (
                                    <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                        <div className="bg-slate-50 p-[8px] sm:px-4 sm:py-3 border-b border-slate-100 font-bold text-slate-700 flex justify-between items-center text-xs sm:text-base">
                                            💸 Expenses
                                            <span className="bg-slate-200 text-slate-600 text-[10px] sm:text-xs px-[8px] py-0.5 rounded-full">{data.expenses.length}</span>
                                        </div>
                                        <div className="p-[8px] space-y-[8px] max-h-48 sm:max-h-64 overflow-y-auto custom-scrollbar">
                                            {data.expenses.map((ex: any) => (
                                                <div key={ex.id} className="p-[8px] sm:p-3 border border-slate-100 rounded-lg sm:rounded-xl hover:border-rose-100 bg-white transition-colors">
                                                    <div className="flex justify-between items-start mb-1">
                            ...
                            {data?.attendance && data.attendance.length > 0 && (
                                <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                    <div className="bg-slate-50 p-[8px] sm:px-4 sm:py-3 border-b border-slate-100 font-bold text-slate-700 flex justify-between items-center text-xs sm:text-base">
                                        📅 Attendance
                                        <span className="bg-slate-200 text-slate-600 text-[10px] sm:text-xs px-[8px] py-0.5 rounded-full">{data.attendance.length}</span>
                                    </div>
                                    <div className="p-[8px] sm:p-4 flex flex-wrap gap-[8px] sm:gap-2">
                                        {data.attendance.map((a: any, idx: number) => (
                                            <div key={idx} className={`p-[8px] sm:px-3 sm:py-1.5 rounded-md sm:rounded-lg border text-[10px] sm:text-xs font-bold flex items-center gap-[8px] sm:gap-2 ${
                            ...
                            {!data?.invoices?.length && !data?.expenses?.length && !data?.attendance?.length && (
                                <div className="text-center p-[8px] sm:py-16 bg-white rounded-2xl sm:rounded-3xl border border-slate-100 border-dashed">
