...
"[project]/Desktop/bill/app/api/products/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const result = await client.query('SELECT * FROM products WHERE created_by = $1 ORDER BY created_at DESC', [
            session.user.id
        ]);
        client.release();
...
async function POST(request) {
    ...
    try {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO products (id, name, description, hsn_code, unit, price, purchase_price, gst_rate, stock_quantity, created_by, type, created_at) 
...
async function PUT(request) {
    ...
    try {
        ...
        const checkOwner = await client.query('SELECT created_by FROM products WHERE id = $1', [
            id
        ]);
...
async function DELETE(request) {
    ...
    try {
        ...
        const checkUsage = await client.query('SELECT COUNT(*) FROM invoice_items WHERE product_id = $1', [
            id
        ]);
        if (parseInt(checkUsage.rows[0].count) > 0) {
            client.release();
            return __TURBOPACK__imported__module__$5b$project$5d2f$Desktop$2f$bill$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'Cannot delete product: It is used in existing invoices'
            }, {
                status: 400
            });
        }
        // Only delete if it belongs to user
        const result = await client.query('DELETE FROM products WHERE id = $1 AND created_by = $2', [
            id,
            userId
        ]);
        client.release();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__fe8280df._.js.map