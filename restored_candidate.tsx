      ...
      <main className="max-w-7xl mx-auto p-4 md:p-10 space-y-8">
        ...
        <div className="flex items-center gap-2 text-slate-500 font-bold text-xs bg-slate-100 w-fit px-4 py-1.5 rounded-full border border-slate-200">
          ...
          <span suppressHydrationWarning>
            {currentTime.toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short' })} GÇó {currentTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
          </span>
        </div>
        ...
        <div className="py-2">
          <h2 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tight">
            {getGreeting()}, <span className="text-orange-500">{businessProfile.name || 'Owner'}</span>! =ƒæï
          </h2>
        </div>
          ...
          <div className="bg-slate-900 rounded-[40px] shadow-2xl p-8 text-white flex flex-col justify-center">
            <div className="space-y-8">
              <div>
                ...
                <h5 className="text-5xl font-black tabular-nums italic tracking-tighter">Gé¦{totalSales.toLocaleString()}</h5>
              </div>
