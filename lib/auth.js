        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    if (!user) {
                        console.log('Auth Debug [Authorize]: User not found', credentials.email);
                        return null;
                    }
                    ...
                    if (!isPasswordValid) {
                        console.log('Auth Debug [Authorize]: Invalid password for', credentials.email);
                        return null;
                    }

                    console.log('Auth Debug [Authorize]: Success for', credentials.email, 'ID:', user.id);
                    return {
                        id: user.id.toString(),
                        email: user.email,
                        name: user.name,
                    ...
                    console.error('Auth Debug [Authorize]: Database error:', error);
                    return null;
                }
            }
        })
    ...
    callbacks: {
        ...
        async session({ session, token }) {
            ...
            if (session?.user) {
                ...
                console.log('Auth Debug [Session]: Assigned ID', session.user.id);
            } else {
                console.warn('Auth Debug [Session]: session.user is MISSING');
            }
            return session;
        }
    },
