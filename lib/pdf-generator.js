...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        // Branding for non-premium
        if (!isPremium) {
            doc.setFontSize(8);
            doc.setTextColor(150);
            doc.text('Invoice generated via BillGST.in - Create your own GST bills at www.billgst.in', pageWidth / 2, pageHeight - margin + 3, { align: 'center' });
        }
...
export const generateHisaabPDF = async (customer, businessDetails, custStats, autoSave = true) => {
    try {
        ...
        if (!isPremium) {
            doc.setFontSize(7);
            doc.setTextColor(150);
            doc.text('Generated via BillGST.in - Get your professional Ledger at www.billgst.in', pageWidth / 2, pageHeight - margin + 3, { align: 'center' });
        }
