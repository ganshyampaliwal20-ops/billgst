...
export const generateInvoicePDF = (invoice, businessDetails) => {
    ...
    doc.text(invoice.customer.name, 15, 60);

    doc.setFont('helvetica', 'bold');
    doc.text(`Invoice No.: ${invoice.invoice_number}`, 195, 55, { align: 'right' });
    doc.text(`Date: ${new Date(invoice.invoice_date).toLocaleDateString('en-GB')}`, 195, 60, { align: 'right' });
    ...
    autoTable(doc, {
        ...
        body: invoice.items.map((item, index) => [
            ...
            `₹ ${parseFloat(item.total_amount || (item.quantity * item.unit_price)).toFixed(2)}`
        ]),
        theme: 'grid',
        headStyles: {
    ...
    const drawRow = (label, value, y, isBold = false) => {
        ...
        doc.text(`₹ ${parseFloat(value).toFixed(2)}`, valueX - 2, y, { align: 'right' });
    };

    drawRow('Sub Total', invoice.total_amount, finalY + 8);
    drawRow('Total', invoice.total_amount, finalY + 14, true);
    drawRow('Received', invoice.paid_amount || 0, finalY + 20);
    drawRow('Balance', invoice.total_amount - (invoice.paid_amount || 0), finalY + 26);

    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text(`PAYMENT MODE: ${(invoice.payment_mode || 'Credit').toUpperCase()}`, summaryX + 2, finalY + 32);
    ...
    if (businessDetails.upi_id) {
        try {
            const upiUrl = `upi://pay?pa=${businessDetails.upi_id}&pn=${encodeURIComponent(businessDetails.name)}&am=${invoice.total_amount}&cu=INR`;
