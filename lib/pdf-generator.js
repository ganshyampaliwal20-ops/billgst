...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        if (autoSave && typeof window !== 'undefined') {
            doc.save(`Invoice_${invoice.invoice_number || '001'}.pdf`);
        }
        return doc;
    } catch (e) {
...
export const generateHisaabPDF = async (customer, businessDetails, custStats, autoSave = true) => {
    try {
        ...
        autoTable(doc, {
            startY: summaryY + 25,
            head: [['DATE', 'DESCRIPTION', 'TYPE', 'CREDIT (IN)', 'DEBIT (OUT)']],
            body: sortedTxns.map((t) => {
                ...
                return [
                    ...
                    desc,
                    t.type.toUpperCase(),
                    t.type === 'credit' ? Number(t.amt).toFixed(2) : '-',
                    t.type !== 'credit' ? Number(t.amt).toFixed(2) : '-'
                ];
            }),
            theme: 'grid',
            headStyles: { fillColor: [93, 80, 136], textColor: 255, fontSize: 9, fontStyle: 'bold' },
            styles: { fontSize: 8 },
            margin: { left: margin, right: margin }
        });
