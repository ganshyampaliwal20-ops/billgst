...
export const generateInvoicePDF = (invoice, businessDetails) => {
    ...
    doc.text(businessDetails.name, 15, 20);
    ...
    const rightColX = 130;  // Moved left for better spacing
    const valueX = 185;     // Changed from 195 to prevent cutoff
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text('Total Amount:', rightColX, finalY + 30);
    doc.text(formatCurrency(invoice.total_amount), valueX, finalY + 30, { align: 'right' });

    // Amount in Words
    doc.setFontSize(10);
    doc.setFont('helvetica', 'italic');
    doc.text(`Amount in Words: ${numberToWords(Math.round(invoice.total_amount))}`, 15, finalY + 30);
