...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        // --- Bank Details (Left side, above or below QR) ---
        if (businessDetails.show_bank_details && (businessDetails.bank_name || businessDetails.account_no)) {
            const bankX = margin + 5;
            let bankY = doc.internal.pageSize.height - margin - 45; // Position above QR or branding
            ...
            if (businessDetails.bank_name) {
                doc.text(`Bank: ${businessDetails.bank_name}`, bankX, bankY);
                bankY += 3.5;
            }
            if (businessDetails.account_holder) {
                doc.text(`A/C Holder: ${businessDetails.account_holder}`, bankX, bankY);
                bankY += 3.5;
            }
            if (businessDetails.account_no) {
                doc.setFont('helvetica', 'bold');
                doc.text(`A/C No: ${businessDetails.account_no}`, bankX, bankY);
                doc.setFont('helvetica', 'normal');
                bankY += 3.5;
            }
            if (businessDetails.ifsc_code) {
                doc.text(`IFSC: ${businessDetails.ifsc_code}`, bankX, bankY);
                bankY += 3.5;
            }
            if (businessDetails.branch_name) {
                doc.text(`Branch: ${businessDetails.branch_name}`, bankX, bankY);
            }
        }
