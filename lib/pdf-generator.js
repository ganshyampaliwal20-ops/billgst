...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true, action = 'view') => {
    try {
        ...
        const isPremium = false; // ['BASIC_30', 'PREMIUM_99', 'YEARLY_299', 'LIFETIME'].includes(businessDetails.plan_type);
...
export const generateHisaabPDF = async (customer, businessDetails, custStats, autoSave = true, action = 'view') => {
    try {
        ...
        // Branding
        // TEMPORARY: Force false to always show watermark
        const isPremium = false; // businessDetails?.subscription_plan === 'PREMIUM' || businessDetails?.subscription_plan === 'ENTERPRISE' || ['BASIC_30', 'PREMIUM_99', 'YEARLY_299', 'LIFETIME'].includes(businessDetails?.plan_type);
        if (!isPremium) {
