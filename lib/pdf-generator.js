...
export const drawFreeBranding = async (doc, isThermal, pageWidth, pageHeight, startY) => {
    try {
        ...
        let yPos = startY || (pageHeight - 15);
        if (isThermal && !startY) yPos = pageHeight - 8; // Adjust for thermal if no specific Y provided

        if (logoB64) {
            const logoSize = isThermal ? 8 : 12;
            doc.addImage(logoB64, 'PNG', (pageWidth / 2) - (logoSize / 2), yPos, logoSize, logoSize);
            doc.setFontSize(isThermal ? 6 : 8);
            doc.setTextColor(150);
            doc.text('Created for free using BillGST.in App', pageWidth / 2, yPos + logoSize + (isThermal ? 3 : 4), { align: 'center' });
        } else {
            doc.setFontSize(isThermal ? 6 : 8);
            doc.setTextColor(150);
            doc.text('Created for free using BillGST.in App', pageWidth / 2, yPos + 4, { align: 'center' });
        }
    } catch(e) { }
};

const autoSavePDF = async (doc, fileName) => {
    ...
    try {
        ...
        console.error('Capacitor Native Save error', e);
    }

    if (nativeSaved) return;

    // Next, try Web Share API if on mobile (helps bypass silent WebView/Chrome downloads)
    if (/Android|webOS|iPhone|iPad|iPod/i.test(navigator.userAgent) && navigator.canShare) {
        try {
            ...
            if (navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: fileName,
                    text: 'Your Invoice PDF'
                });
                return; // If shared successfully, we are done
            }
        } catch (e) {
    ...
    // Final fallback for desktop or if Share fails
    doc.save(fileName);
};
...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        const doc = jsPDF({ orientation: 'p', unit: 'mm', format: formatOpt });
        ...
        const hasQR = Boolean(businessDetails.upi_id && invoice.total_amount > 0);
        ...
        // Branding
        if (!isPremium) {
            await drawFreeBranding(doc, isThermal, pageWidth, pageHeight, isThermal ? sigY + 22 : bottomBoxLimit - 18);
        }

        if (autoSave && typeof window !== 'undefined') {
            await autoSavePDF(doc, `Invoice_${invoice.invoice_number || '001'}.pdf`);
        }
        return doc;
    } catch (e) {
...
export const generateHisaabPDF = async (customer, businessDetails, custStats, autoSave = true) => {
    try {
        ...
        // Branding
        const isPremium = businessDetails?.subscription_plan === 'PREMIUM' || businessDetails?.subscription_plan === 'ENTERPRISE' || ['BASIC_30', 'PREMIUM_99', 'YEARLY_299', 'LIFETIME'].includes(businessDetails?.plan_type);
        if (!isPremium) {
            await drawFreeBranding(doc, false, pageWidth, pageHeight, pageHeight - margin - 18);
        }

        if (autoSave && typeof window !== 'undefined') {
            await autoSavePDF(doc, `Ledger_${customer.name}.pdf`);
        }
        return doc;
    } catch (e) {
...
export const generateCatalogPDF = async (products, businessDetails, autoSave = true) => {
    try {
        ...
        if (autoSave && typeof window !== 'undefined') {
            await autoSavePDF(doc, `Product_Catalog.pdf`);
        }
        return doc;
    } catch (e) {
