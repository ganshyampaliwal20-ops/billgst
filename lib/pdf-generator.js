...
// ==========================================
// PDF SETTINGS (Change these to fix Cutoff)
// ==========================================
const PDF_CONFIG = {
    // Column Widths (Increase 'amount' if numbers are cutting off)
    itemNumberWidth: 10,
    productNameWidth: 48,
    hsnWidth: 20,
    quantityWidth: 16,
    priceWidth: 23,
    gstWidth: 15,
    amountWidth: 42, // <--- Change this if Amount cuts off (e.g., make it 45 or 50)

    // Total Amount Position (Right side)
    totalLabelX: 105,
    totalValueX: 188, // <--- Increase this if Total Amount cuts off (e.g., 190 or 195)
};
...
export const generateInvoicePDF = (invoice, businessDetails) => {
    ...
    autoTable(doc, {
        ...
        columnStyles: {
            0: { cellWidth: PDF_CONFIG.itemNumberWidth },
            1: { cellWidth: PDF_CONFIG.productNameWidth },
            2: { cellWidth: PDF_CONFIG.hsnWidth },
            3: { cellWidth: PDF_CONFIG.quantityWidth },
            4: { cellWidth: PDF_CONFIG.priceWidth },
            5: { cellWidth: PDF_CONFIG.gstWidth },
            6: { cellWidth: PDF_CONFIG.amountWidth, halign: 'right' }
        }
    });

    const finalY = doc.lastAutoTable.finalY + 10;

    // Totals Section - Using Config
    const rightColX = PDF_CONFIG.totalLabelX;
    const valueX = PDF_CONFIG.totalValueX;
