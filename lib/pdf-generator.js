...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        const isPremium = ['PREMIUM_99', 'YEARLY_299', 'LIFETIME'].includes(businessDetails.plan_type);
...
export const generateHisaabPDF = async (customer, businessDetails, custStats, autoSave = true) => {
    try {
        ...
        doc.text(String(businessDetails?.name || 'Hisaab Pro').toUpperCase(), pageWidth / 2, margin + 10, { align: 'center' });
        ...
        doc.setFontSize(7);
        doc.setTextColor(150);
        doc.text('Generated via BillGST.in - Hisaab Pro', pageWidth / 2, pageHeight - margin + 3, { align: 'center' });

        if (autoSave && typeof window !== 'undefined') {
            doc.save(`Hisaab_${customer.name}.pdf`);
        }
        return doc;
    } catch (e) {
