...
export const drawFreeBranding = async (doc, isThermal, pageWidth, pageHeight, startY) => {
    try {
        ...
        if (logoB64) {
            const logoSize = isThermal ? 6 : 7; // Smaller logo to fit inside the bottom margin
            doc.addImage(logoB64, 'PNG', (pageWidth / 2) - (logoSize / 2), yPos, logoSize, logoSize);
            doc.setFontSize(isThermal ? 5 : 6);
            doc.setTextColor(150);
            doc.text('Created for free using BillGST.in', pageWidth / 2, yPos + logoSize + 2.5, { align: 'center' });
        } else {
            doc.setFontSize(isThermal ? 5 : 6);
            doc.setTextColor(150);
            doc.text('Created for free using BillGST.in', pageWidth / 2, yPos + 3, { align: 'center' });
        }
    } catch(e) { }
};
