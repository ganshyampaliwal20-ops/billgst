...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        // Use fallback if missing for testing
        const upiIdToUse = businessDetails.upi_id || 'test@upi';

        if (invoice.total_amount > 0) {
            try {
                const upiAmount = (Number(invoice.total_amount) || 0).toFixed(2);
                const upiLink = `upi://pay?pa=${upiIdToUse}&pn=${encodeURIComponent(businessDetails.name)}&am=${upiAmount}&cu=INR`;
                ...
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(6);
                const displayId = businessDetails.upi_id ? businessDetails.upi_id : 'Update UPI in Settings';
                doc.text(`UPI: ${displayId}`, qrX + (qrSize / 2), qrY + qrSize + 3, { align: 'center' });
                ...
                doc.setFontSize(8);
                doc.text(`QR Error: ${qrError.message}`, margin, doc.internal.pageSize.height - 20);
            }
        }
        ...
        // UNCONDITIONAL DEBUG: FORCE DRAW COMPONENT
        // This runs regardless of total_amount or UPI ID
        doc.setDrawColor(255, 0, 0); // Red
        doc.setLineWidth(1);
        const debugX = margin + 40;
        const debugY = doc.internal.pageSize.height - margin - 25;
        doc.rect(debugX, debugY, 25, 25); // Square box next to where QR should be

        doc.setTextColor(255, 0, 0);
        doc.setFontSize(8);
        doc.text(`DEBUG: Amt=${invoice.total_amount} | UPI=${businessDetails.upi_id}`, debugX, debugY - 2);
