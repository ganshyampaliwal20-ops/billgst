...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true) => {
    try {
        ...
        // --- Header Logic based on Format ---
        const tableFormat = businessDetails.invoice_table_format || 'FORMAT_1';

        let headerEndY = margin + 35; // Default end of header

        if (tableFormat === 'FORMAT_2') {
            // --- HEADER STYLE: PHOTO MATCH (Logo Left, Text Center) ---

            // 1. Top Title
            doc.setFontSize(10);
            doc.setFont(theme.font, 'bold');
            doc.setTextColor(theme.title[0], theme.title[1], theme.title[2]);
            const titleText = DOC_LABELS[invoice.type] || 'TAX INVOICE';
            doc.text(titleText, pageWidth / 2, margin + 5, { align: 'center' });

            // 2. Logo (Left)
            if (businessDetails.logo) {
                try {
                    doc.addImage(businessDetails.logo, 'PNG', margin + 2, margin + 8, 22, 22);
                } catch (e) { }
            }

            // 3. Business Details (Center)
            doc.setTextColor(0, 0, 0);
            doc.setFontSize(16);
            doc.setFont(theme.font, 'bold');
            const busName = String(businessDetails.name || 'Your Business Name').toUpperCase();
            doc.text(busName, pageWidth / 2, margin + 14, { align: 'center' });

            doc.setFontSize(9);
            doc.setFont(theme.font, 'normal');
            const busAddr = doc.splitTextToSize(businessDetails.address || '', 120);
            doc.text(busAddr, pageWidth / 2, margin + 19, { align: 'center' });
            ...
            doc.text(contactText, pageWidth / 2, margin + 25, { align: 'center' });

            if (businessDetails.gstin) {
                doc.setFont(theme.font, 'bold');
                doc.text(`GSTIN: ${businessDetails.gstin}`, pageWidth / 2, margin + 29, { align: 'center' });
            }

            // Original For Buyer text
            doc.setFontSize(8);
            doc.setFont(theme.font, 'italic');
            doc.text('Original For Buyer', pageWidth / 2, margin + 33, { align: 'center' });
            ...
            // Don't print the standard title later
            headerEndY = margin + 36;

        } else {
            // --- HEADER STYLE: STANDARD (Logo Right, Text Left) ---
            doc.setFont(theme.font, 'bold');
            doc.setFontSize(14);
            const busName = String(businessDetails.name || 'Your Business Name').toUpperCase();
            ...
            headerEndY = margin + 40; // Adjust for standard
        }
