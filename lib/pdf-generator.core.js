...
export const shouldShowBranding = (businessDetails) => {
    return !['BASIC_30', 'PREMIUM_99', 'YEARLY_299', 'LIFETIME'].includes(businessDetails?.plan_type);
};
...
import { generateVyaparInvoicePDF } from './pdf-generator-vyapar';
import { generateEwayBillPDF } from './pdf-generator-ewaybill';
...
export const drawFreeBranding = async (doc, isThermal, pageWidth, pageHeight, startY) => {
    try {
        ...
        let yPos = pageHeight - 6; // Absolute bottom of the A4/A5 page
        if (isThermal && startY) yPos = startY;
        ...
        if (logoB64) {
            ...
            try {
                ...
                doc.addImage(logoB64, format, startX, yPos, logoSize, logoSize);
            } catch(imgErr) {
                console.error("Failed to add app logo:", imgErr);
            }
...
export const generateInvoicePDF = async (invoice, businessDetailsArg, autoSave = true, action = 'view', externalToastId = null) => {
    const templateId = businessDetailsArg?.invoice_template || 'TEMPLATE_1';
    
    if (invoice?.type === 'E_WAY_BILL') {
        return generateEwayBillPDF(invoice, businessDetailsArg || {}, autoSave, action, externalToastId);
    }
    
    if (templateId === 'TEMPLATE_8') {
        return generateVyaparInvoicePDF(invoice, businessDetailsArg || {}, autoSave, action, externalToastId);
    }
    // If caller already shows a toast, reuse it to avoid duplicate toasts
    const toastId = externalToastId 
        ? (toast.loading('Generating PDF...', { id: externalToastId }), externalToastId)
    ...
    try {
        ...
        if (logoPos === 'CENTER') {
            ...
            if (businessDetails.logo) {
                try { 
                    const logoFormat = businessDetails.logo.includes('image/png') ? 'PNG' : 'JPEG';
                    const logoSize = isThermal ? 15 : 22;
                    doc.addImage(businessDetails.logo, logoFormat, (pageWidth / 2) - (logoSize/2), margin + 8, logoSize, logoSize); 
                } catch (e) { }
            }
            ...
            if (businessDetails.logo) {
                try { 
                    const logoFormat = businessDetails.logo.includes('image/png') ? 'PNG' : 'JPEG';
                    doc.addImage(businessDetails.logo, logoFormat, margin + 2, margin + 4, 18, 18); 
                } catch (e) { }
            }
            ...
            if (businessDetails.logo) {
                try { 
                    const logoFormat = businessDetails.logo.includes('image/png') ? 'PNG' : 'JPEG';
                    doc.addImage(businessDetails.logo, logoFormat, pageWidth - margin - 22, margin + 4, 18, 18); 
                } catch (e) { }
            }
        ...
        // E-Way Bill Details
        if (invoice.eway_bill_no && !isThermal) {
            doc.setFontSize(8);
            doc.setFont(theme.font, 'bold');
            doc.text('E-Way Bill Details:', margin + 2, startTableY - 4);
            doc.setFont(theme.font, 'normal');
            doc.setFontSize(7);
            doc.text(`EWB No: ${invoice.eway_bill_no} | Vehicle: ${invoice.vehicle_no || '-'} | Transporter: ${invoice.transporter_name || '-'} (${invoice.transporter_id || '-'}) | Mode: ${invoice.transport_mode || '-'} | Distance: ${invoice.distance || '-'} Km`, margin + 2, startTableY);
            startTableY += 5;
        }
        ...
        const thisBal = Math.max(0, Number(invoice.total_amount) - paidAmount);
        const currentBal = prevBal + thisBal;

        if (paidAmount > 0) {
            sY = row(`Paid (${paymentMode})`, paidAmount, sY);
        }
        
        sY = row('Previous Balance', prevBal, sY);
        if (currentBal > 0) {
            sY = row('Current Balance', currentBal, sY, true);
        } else {
            sY = row('Current Balance', 0, sY);
        }
        ...
        const isPremiumForQR = isUserPremium(businessDetails);
        ...
        if (businessDetails.signature) {
            try {
                ...
                const sigFormat = businessDetails.signature.includes('image/png') ? 'PNG' : 'JPEG';
                doc.addImage(businessDetails.signature, sigFormat, pageWidth - margin - (signW + 5), sigY + 2, signW, signH);
            } catch (e) { }
        }
        ...
        // Branding
        if (shouldShowBranding(businessDetails)) {
...
export const generateQuotationPDF = async (quotation, businessDetailsArg, autoSave = true, action = 'view') => {
    ...
    return generateInvoicePDF({ ...quotation, type: 'QUOTATION' }, businessDetailsArg, autoSave, action, toastId);
};
