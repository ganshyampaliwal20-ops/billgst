    ...
    persist(
        (set, get) => ({
            ...
            fetchInvoices: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().invoices && get().invoices.length > 0) return;
                try {
            ...
            fetchCustomers: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().customers && get().customers.length > 0) return;
                try {
            ...
            fetchProducts: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().products && get().products.length > 0) return;
                try {
