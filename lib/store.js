    ...
    persist(
        (set, get) => ({
            ...
            fetchInvoices: async (force = false) => {
                if (!force && get().invoices && get().invoices.length > 0) return;
                try {
            ...
            fetchCustomers: async (force = false) => {
                if (!force && get().customers && get().customers.length > 0) return;
                try {
            ...
            fetchProducts: async (force = false) => {
                if (!force && get().products && get().products.length > 0) return;
                try {
            ...
            fetchQuotations: async (force = false) => {
                if (!force && get().quotations && get().quotations.length > 0) return;
                try {
            ...
            fetchExpenses: async (force = false) => {
                if (!force && get().expenses && get().expenses.length > 0) return;
                try {
            ...
            fetchStaff: async (force = false) => {
                if (!force && get().staff && get().staff.length > 0) return;
                try {
                    const res = await fetch('/api/staff');
                    if (res.status === 401) return;
                    const data = await res.json();
                    if (Array.isArray(data)) set({ staff: data });
                } catch (error) {
            ...
            markAttendance: async (staff_id, date, status, in_time = null, out_time = null, note = null) => {
                try {
                    ...
                    // Optimistic update: filter out exact staff+date combination, then add new one
                    const existing = get().attendance.filter(a => !(a.staff_id === staff_id && a.date === date));
