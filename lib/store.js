...
export const useStore = create(
    persist(
        (set, get) => ({
            ...
            markAttendance: async (staff_id, date, status, in_time = null, out_time = null) => {
                try {
                    const res = await fetch('/api/attendance', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ staff_id, date, status, in_time, out_time })
                    });
                    ...
                    set({ attendance: [...existing, { staff_id, date, status, in_time, out_time }] });
