    ...
    persist(
        (set, get) => ({
            ...
            updateProduct: async (id, data) => {
                try {
                    ...
                    const fullData = { ...currentProduct, ...data };
        ...
        }),
        {
            ...
            storage: createJSONStorage(() => {
                if (typeof window !== 'undefined') {
                    try {
                        // Attempt to access localStorage to see if it's available
                        window.localStorage.setItem('__test', '1');
                        window.localStorage.removeItem('__test');
                        return window.localStorage;
                    } catch (e) {
                        console.warn('localStorage is blocked or unavailable, using in-memory storage fallback');
                        // Return in-memory fallback if localStorage throws DOMException
