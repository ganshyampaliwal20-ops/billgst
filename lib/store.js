    ...
    persist(
        (set, get) => ({
            ...
            fetchInvoices: async (force = false, page = 1) => {
                const now = Date.now();
                if (!force && page === 1 && get().invoices?.length > 0 && (now - (get().lastFetched?.invoices || 0) < 120000)) {
                    return; // Use local cache if fetched within last 2 minutes to save API costs
                }
                ...
                try {
                    ...
                    if (Array.isArray(data)) {
                        if (page === 1) {
                            if (JSON.stringify(get().invoices) !== JSON.stringify(data)) {
                                set(state => ({ invoices: data, lastFetched: { ...state.lastFetched, invoices: now } }));
                            } else {
                                set(state => ({ lastFetched: { ...state.lastFetched, invoices: now } }));
                            }
                        } else {
            ...
            fetchCustomers: async (force = false, page = 1) => {
                const now = Date.now();
                if (!force && page === 1 && get().customers?.length > 0 && (now - (get().lastFetched?.customers || 0) < 120000)) {
                    return; // Local cache (2 mins TTL)
                }

                try {
                    const res = await fetch(`/api/customers?page=${page}&limit=500`);
                    ...
                    if (Array.isArray(data)) {
                        if (page === 1) {
                            if (JSON.stringify(get().customers) !== JSON.stringify(data)) {
                                set(state => ({ customers: data, lastFetched: { ...state.lastFetched, customers: now } }));
                            } else {
                                set(state => ({ lastFetched: { ...state.lastFetched, customers: now } }));
                            }
                        } else {
                        ...
                        console.error('Invalid customers data received:', data);
                    }
                } catch (error) {
            ...
            fetchProducts: async (force = false, page = 1) => {
                const now = Date.now();
                if (!force && page === 1 && get().products?.length > 0 && (now - (get().lastFetched?.products || 0) < 120000)) {
                    return; // Local cache (2 mins TTL)
                }

                try {
                    const res = await fetch(`/api/products?page=${page}&limit=500`);
                    if (res.status === 401) return;
                    const data = await res.json();
                    if (Array.isArray(data)) {
                        if (page === 1) {
                            if (JSON.stringify(get().products) !== JSON.stringify(data)) {
                                set(state => ({ products: data, lastFetched: { ...state.lastFetched, products: now } }));
                            } else {
                                set(state => ({ lastFetched: { ...state.lastFetched, products: now } }));
                            }
                        } else {
            ...
            addProduct: async (product, opts = {}) => {
                try {
                    ...
                    if (savedProduct.id) {
                        ...
                        if (!opts.silent) {
                            toast.success('Product added successfully', { id: 'product_added' });
                        }
                        return savedProduct;
                    }
                } catch (error) {
            ...
            updateProduct: async (id, data, opts = {}) => {
                try {
                    ...
                    if (!opts.silent) {
                        toast.success('Product updated successfully', { id: 'product_updated' });
                    }
                    return { success: true };
                } catch (error) {
            ...
            fetchQuotations: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().quotations && get().quotations.length > 0) return;
                try {
                    const res = await fetch(`/api/quotations?page=${page}&limit=500`);
                    if (res.status === 401) return;
                    const data = await res.json();
                    if (Array.isArray(data)) {
                        if (page === 1) {
            ...
            fetchExpenses: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().expenses && get().expenses.length > 0) return;
                try {
                    const res = await fetch(`/api/expenses?page=${page}&limit=500`);
                    if (res.status === 401) return;
                    const data = await res.json();
                    if (Array.isArray(data)) {
                        if (page === 1) {
            ...
            fetchStaff: async (force = false, page = 1) => {
                // Removed to fix cross-device sync: if (!force && page === 1 && get().staff && get().staff.length > 0) return;
                try {
                    const res = await fetch(`/api/staff?page=${page}&limit=500&t=${Date.now()}`);
                    if (res.status === 401) return;
                    const data = await res.json();
                    if (Array.isArray(data)) {
                        if (page === 1) {
                            if (JSON.stringify(get().staff) !== JSON.stringify(data)) {
                                set({ staff: data });
                            }
                        } else {
            ...
            addStaff: async (member) => {
                try {
                    ...
                    const newStaff = { ...member, id: result.id, is_deleted: false, created_at: new Date().toISOString() };
                    set({ staff: [newStaff, ...get().staff] });
                    await get().fetchStaff(true);
            ...
            deleteStaff: async (id) => {
                try {
                    ...
                    set({ staff: get().staff.map(s => s.id === id ? { ...s, is_deleted: true } : s) });
            ...
            setAiCopilotAction: (action) => set({ aiCopilotAction: action }),
            updateAiCopilotStep: (stepIndex, status, subtitle) => set((state) => {
                if (!state.aiCopilotAction) return {};
                const steps = [...(state.aiCopilotAction.steps || [])];
                if (steps[stepIndex]) {
                    steps[stepIndex] = { ...steps[stepIndex], status: status || 'done' };
                }
                const currentStep = stepIndex + (status === 'done' ? 1 : 0);
                const progress = Math.min(100, Math.round(((stepIndex + (status === 'done' ? 1 : 0.5)) / Math.max(1, steps.length)) * 100));
                return {
                    aiCopilotAction: {
                        ...state.aiCopilotAction,
                        steps,
                        currentStep,
                        progress,
                        subtitle: subtitle || state.aiCopilotAction.subtitle,
                        isVisible: true
                    }
                };
            }),
            completeAiCopilotAction: (successMsg) => set((state) => {
                if (!state.aiCopilotAction) return {};
                const steps = (state.aiCopilotAction.steps || []).map(s => ({ ...s, status: 'done' }));
                return {
                    aiCopilotAction: {
                        ...state.aiCopilotAction,
                        steps,
                        ...
                        progress: 100,
                        subtitle: successMsg || 'Kaam poora ho gaya! ✅',
                        isComplete: true,
                        isVisible: true
                    }
                };
            }),
            clearAiCopilotAction: () => set({ aiCopilotAction: null }),
        }),
        {
