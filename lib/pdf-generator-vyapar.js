...
export const generateVyaparInvoicePDF = async (invoice, businessDetails, autoSave = true, action = 'view') => {
    const toastId = toast.loading('Generating PDF (Vyapar Style)...');
    try {
        if (!invoice) {
        ...
        // --- Table ---
        const tableCols = ['SR NO.', 'Item name', 'Quantity', 'Unit', 'Price/Unit(₹)', 'Amount(₹)'];
        const tableRows = [];
        let totalQty = 0;
        ...
        autoTable(doc, {
            ...
            foot: [['Total', '', totalQty.toFixed(1), '', '', `₹ ${Number(invoice.total_amount || 0).toFixed(2)}`]],
        ...
        doc.text(`₹ ${Number(invoice.total_amount || 0).toFixed(2)}`, pageWidth - margin - 2, currY + 4, { align: 'right' });
        ...
        doc.text(`₹ ${Number(invoice.total_amount || 0).toFixed(2)}`, pageWidth - margin - 2, currY + 11, { align: 'right' });
        ...
        doc.text(`${inWords} Rupees only`, pageWidth / 2 + 2, currY + 22);
        ...
        doc.text(`₹ ${Number(invoice.paid_amount || 0).toFixed(2)}`, pageWidth - margin - 2, currY + 28, { align: 'right' });

        doc.text('Current Balance', pageWidth / 2 + 2, currY + 34);
        doc.text(':', pageWidth / 2 + 30, currY + 34);
        doc.text(`₹ ${bal.toFixed(2)}`, pageWidth - margin - 2, currY + 34, { align: 'right' });
        ...
        // bottom border is already drawn by main bounding box.
        // wait, we drew the main bounding box with pageHeight, but we should make sure our content fits!
        // actually, let's just make the main bounding box match sigY + sigHeight.
        doc.setDrawColor(255,255,255);
        doc.setLineWidth(1);
        doc.line(margin, sigY + sigHeight + 0.5, pageWidth - margin, sigY + sigHeight + 0.5); // erase the bottom of the old box if it's longer
        doc.setDrawColor(0,0,0);
        doc.setLineWidth(0.2);
        doc.line(margin, sigY + sigHeight, pageWidth - margin, sigY + sigHeight); // draw new bottom line
        ...
        if (action === 'download') {
            doc.save(fileName);
        } else if (action === 'share' || action === 'print') {
            const pdfBlob = doc.output('blob');
            const file = new File([pdfBlob], fileName, { type: 'application/pdf' });
            
            if (action === 'share') {
                await downloadAndShareFile(file, fileName);
            } else {
