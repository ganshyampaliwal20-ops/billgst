    ...
    try {
        ...
        if (businessDetails.logo) {
            try {
                ...
                doc.addImage(businessDetails.logo, logoFormat, margin + 2, startY, 15, 15);
            } catch (e) {
        ...
        // Vertical split line
        doc.line(pageWidth / 2, currY, pageWidth / 2, currY + 42);
        ...
        const thisInvoiceBal = Math.max(Number(invoice.total_amount || 0) - received, 0);
        const currentBal = prevBal + thisInvoiceBal;

        doc.setFontSize(6);
        doc.text('Received', pageWidth / 2 + 2, currY + 28);
        doc.text(':', pageWidth / 2 + 30, currY + 28);
        doc.text(`Rs. ${received.toFixed(2)}`, pageWidth - margin - 2, currY + 28, { align: 'right' });

        doc.text('Previous Balance', pageWidth / 2 + 2, currY + 34);
        doc.text(':', pageWidth / 2 + 30, currY + 34);
        doc.text(`Rs. ${prevBal.toFixed(2)}`, pageWidth - margin - 2, currY + 34, { align: 'right' });

        doc.text('Current Balance', pageWidth / 2 + 2, currY + 40);
        doc.text(':', pageWidth / 2 + 30, currY + 40);
        doc.text(`Rs. ${currentBal.toFixed(2)}`, pageWidth - margin - 2, currY + 40, { align: 'right' });
        ...
        if (businessDetails.signature) {
            try {
                ...
                doc.addImage(businessDetails.signature, sigFormat, pageWidth - margin - 35, sigY + 6, 25, 12);
            } catch (e) {
        ...
        const { shouldShowBranding, drawFreeBranding } = await import('./pdf-generator.core.js');
        if (shouldShowBranding(businessDetails)) {
            await drawFreeBranding(doc, false, pageWidth, pageHeight, sigY + sigHeight + 3);
        }
