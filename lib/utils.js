    ...
    if (isNativeApp()) {
        ...
        if (Filesystem && Share) {
            try {
                ...
                // Purane PDFs ko cache se hatana zaroori hai taaki Android PDF viewer unhe dobara na khole
                try {
                    const cacheDir = await Filesystem.readdir({ path: '', directory: 'CACHE' });
                    for (const file of cacheDir.files) {
                        if ((file.name || file).endsWith('.pdf')) {
                            await Filesystem.deleteFile({ path: file.name || file, directory: 'CACHE' }).catch(() => {});
                        }
                    }
                } catch(clearErr) {}
                ...
                if (action === 'view') {
                    if (FileOpener) {
                        try {
                            ...
                            // Agar cache clear hone ki wajah se PDF viewer purana file dhoondh kar fail ho jata hai,
                            // to hum turant 400ms baad wapas try karte hain jisse naya PDF khul jaye!
                            try {
                                ...
                                // Agar phir bhi fail ho, to share sheet dikhao
                                await Share.share({
                                    title: fileName,
                                    url: tempFile.uri,
