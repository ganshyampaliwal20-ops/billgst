    ...
    if (isNativeApp()) {
        ...
        if (Filesystem && Share) {
            try {
                ...
                if (action === 'view') {
                    // Android OS ke PDF Viewer me ek purana bug hai.
                    // Agar ek bar PDF khul gaya, to dusra PDF open karne par wo purana wala hi dikhata hai.
                    // Isko fix karne ke liye hum 'Share' dialog ka use karte hain, jo hamesha naya PDF open karta hai.
                    await Share.share({
                        title: fileName,
                        url: tempFile.uri,
