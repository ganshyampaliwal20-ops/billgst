...
export const downloadAndShareFile = async (base64Data, fileName, mimeType = 'application/pdf', action = 'view', windowRef = null) => {
    ...
    LocalNotifications.requestPermissions().then(() => {
        LocalNotifications.schedule({
            notifications: [{
                ...
                if (action === 'view') {
                    if (FileOpener) {
                        // Use FileOpener directly for true native viewing
                        try {
                            await new Promise(r => setTimeout(r, 300));
                            // Fire without awaiting to prevent plugin lockup
                            FileOpener.open({
                                filePath: tempFile.uri,
                                contentType: mimeType,
                                openWithDefault: true
                            }).catch(err => {
                                console.warn('FileOpener error', err);
                                if (Share) Share.share({ title: fileName, url: tempFile.uri });
                            });
                        } catch (e) {}
                        return true;
                    } else if (Share) {
                        try { await Share.share({ title: fileName, url: tempFile.uri }); } catch(e){}
                        return true;
                    }
                } else if (action === 'share' || action === 'download' || action === 'save') {
                    if (Share) {
                        try {
                            await Share.share({
                                title: fileName,
                                url: tempFile.uri,
                                dialogTitle: action === 'share' ? 'Open / Share File' : 'Open File'
                            });
                        } catch (shareErr) {}
                    }
                    return true;
    ...
    try {
        ...
        if (action === 'view') {
            if (windowRef) {
                // If a window is provided (Desktop), navigate it
                windowRef.location.href = url;
                // On mobile web, this triggers a download and leaves a blank tab. Close it shortly after.
                if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                    setTimeout(() => { try { windowRef.close(); } catch(e){} }, 1000);
                }
            } else {
                // To avoid popup blockers blocking subsequent clicks, use a dynamically created link instead of window.open for mobile web
