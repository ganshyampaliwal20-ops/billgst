    ...
    if (isNativeApp()) {
        ...
        if (Filesystem && Share) {
            try {
                ...
                if (action === 'view') {
                    // Due to severe caching bugs in Android PDF viewers with ACTION_VIEW intents
                    // when the viewer is already in the background, we use Share.share (ACTION_SEND)
                    // which consistently forces a fresh intent resolution and loads the new file.
                    await Share.share({
                        title: fileName,
                        url: tempFile.uri,
