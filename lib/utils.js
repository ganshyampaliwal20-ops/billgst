...
export const downloadAndShareFile = async (base64Data, fileName, mimeType = 'application/pdf', action = 'view') => {
    ...
    if (isNativeApp()) {
        ...
        if (Filesystem && Share) {
            try {
                ...
                if (action === 'view') {
                    if (FileOpener) {
                        try {
                            ...
                            // Fallback to Share if FileOpener completely fails, so user can at least access the PDF
                            await Share.share({
                                title: fileName,
                                url: savedFile.uri,
                        ...
                        // FileOpener missing, just let it finish.
                        return true;
                    }
                ...
                // If action is 'save' or 'download', do nothing further. It's successfully saved to DOCUMENTS.
                return true;
            } catch (e) {
    ...
    try {
        ...
        if (action === 'view') {
            ...
            // Clean up the URL after a short delay since we opened it in a new tab
