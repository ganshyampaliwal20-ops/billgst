    ...
    if (isNativeApp()) {
        ...
        if (Filesystem) {
            try {
                ...
                if (action === 'view' || action === 'share' || action === 'download' || action === 'save') {
                    // Always use Share for Native Android View to bypass the notorious Google Drive PDF Viewer caching bug
                    if (Share) {
                        try {
                            await Share.share({ title: fileName, url: tempFile.uri, dialogTitle: action === 'view' ? 'Open PDF with...' : 'Share File' });
                        } catch(e){}
                    } else if (FileOpener) {
