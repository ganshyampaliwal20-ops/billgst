...
export const downloadAndShareFile = async (base64Data, fileName, mimeType = 'application/pdf', action = 'view', windowRef = null) => {
    ...
    if (isNativeApp()) {
        // Using statically imported Capacitor plugins directly for reliability
        ...
        if (Filesystem) {
            try {
                ...
                // Removed background cache clearer to prevent race conditions with FileOpener
                ...
                const tempFile = await Filesystem.writeFile({
                    ...
                    directory: 'CACHE',
                          ...
                          // Only show download notification if action is download/save (prevent confusion on 'view')
                ...
                if (action === 'share') {
                    if (Share) {
                        try { await Share.share({ title: fileName, url: tempFile.uri, dialogTitle: 'Share File' }); } catch(e){}
                    }
                } else if (action === 'view' || action === 'download' || action === 'save') {
                    if (FileOpener) {
                        try { await FileOpener.open({ filePath: tempFile.uri, contentType: mimeType, openWithDefault: true }); } catch (e) {}
                    } else if (Share) {
                        // Fallback to share if FileOpener is missing
                        try { await Share.share({ title: fileName, url: tempFile.uri, dialogTitle: 'Open PDF' }); } catch(e){}
                    }
                }
                return true;
                return true;
    ...
    try {
        ...
        // On Mobile Web, 'view' or 'share' should open the Native Share Sheet because mobile browsers cannot view PDFs inline
        if (isMobile && (action === 'view' || action === 'share') && navigator.canShare && navigator.canShare({ files: [file] })) {
            if (windowRef) {
            ...
            try {
                await navigator.share({
                    files: [file],
                    title: fileName,
                    text: action === 'view' ? 'View PDF' : 'Here is your file'
                });
                return true;
            } catch (err) {
                // Ignore AbortError if user cancelled share sheet
            }
        }
        ...
        if (action === 'view') {
            if (windowRef) {
                // Desktop Web: Navigate the provided window
                windowRef.location.href = url;
            } else {
                if (isMobile) {
                    // Mobile fallback if share failed (downloads it)
                    ...
                    const newWindow = window.open(url, uniqueTarget);
                    if (!newWindow) {
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = fileName;
        ...
        toast.success((t) => React.createElement(
            ...
            React.createElement('span', null, 'File Downloaded'),
            React.createElement(
                'button',
                {
                    onClick: () => { window.open(url, '_blank'); toast.dismiss(t.id); },
                    style: { background: '#4f46e5', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }
                },
                'View'
            )
