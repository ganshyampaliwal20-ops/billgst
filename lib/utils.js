    ...
    if (isNativeApp()) {
        ...
        if (Filesystem) {
            try {
                ...
                // Clear old PDF cache to prevent stale files
                try {
                ...
                if (LocalNotifications) {
                    try {
                        ...
                        console.warn('LocalNotifications schedule error:', notifErr);
                    }
                }
                ...
                if (action === 'view') {
                    ...
                    // For download action on native app, also open share sheet so user can directly open with Google Sheets/Viewer/WhatsApp
                    if (Share) {
                        try {
                            await Share.share({
                                title: fileName,
            ...
            try {
                if (Notification.permission === 'granted') {
                    new Notification('📥 File Downloaded', {
                        body: `${fileName} saved to your Downloads folder.`,
                        icon: '/icon.png'
                    });
